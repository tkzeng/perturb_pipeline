...
def display_tables_paginated(tables: List, filtered_indices: Set[int], 
                            metadata_values: Dict, key_prefix: str):
    """Display tables with pagination and grouping controls"""
    # Get filtered tables
    if filtered_indices is not None:
        filtered_tables = [tables[i] for i in sorted(filtered_indices)]
    else:
    ...
    with col1:
        page = st.number_input("Page", min_value=1, 
                              max_value=max(1, (total_tables - 1) // TABLES_PER_PAGE + 1),
                              value=1, key=f"{key_prefix}_tables_page")
    
    with col2:
        st.write(f"Showing {min((page-1)*TABLES_PER_PAGE + 1, total_tables)}-"
                f"{min(page*TABLES_PER_PAGE, total_tables)} of {total_tables} tables")
    
    # Grouping controls
    st.markdown("---")
    col1, col2, col3 = st.columns([2, 2, 6])
    
    # Extract metadata keys from current page tables
    ...
    for table in page_tables:
        available_group_keys.update(k for k, v in table.items() 
                                  if k not in INTERNAL_FIELDS
                                  and v is not None)
    ...
    with col1:
        group_options = ['None'] + sorted(list(available_group_keys))
        primary_group = st.selectbox("Group by:", group_options, 
                                     key=f"{key_prefix}_tables_primary_group")
    ...
    with col2:
        if primary_group != 'None':
            secondary_options = ['None'] + [k for k in sorted(available_group_keys) if k != primary_group]
            secondary_group = st.selectbox("Then by:", secondary_options,
                                         key=f"{key_prefix}_tables_secondary_group")
        else:
            secondary_group = 'None'
    
    # Display tables for current page
    display_grouped_tables(page_tables, primary_group, secondary_group)

def display_grouped_tables(tables: List, primary_group: str, secondary_group: str):
    """Display tables with grouping"""
    if primary_group == 'None':
        # No grouping - display flat list
        for table in tables:
            display_single_table(table)
    
    elif secondary_group == 'None':
        # Single-level grouping
        groups = defaultdict(list)
        for table in tables:
            group_value = table.get(primary_group, 'Other')
            groups[group_value].append(table)
        ...
        primary_groups = defaultdict(lambda: defaultdict(list))
        for table in tables:
            primary_value = table.get(primary_group, 'Other')
            secondary_value = table.get(secondary_group, 'Other')
            primary_groups[primary_value][secondary_value].append(table)
        ...
        for primary_name in sorted(primary_groups.keys()):
            ...
            for secondary_name in sorted(primary_groups[primary_name].keys()):
                ...
                for table in primary_groups[primary_name][secondary_name]:
                    display_single_table(table)
